...
def dispense(start_weight):
    ...
    while GPIO.input(START) == GPIO.LOW:
        ...
        dispensed = start_weight - hx.get_weight_mean(15)
        #print(dispensed)
        ...
        lcd_string("Dispenser Active",LCD_LINE_1,2)
        lcd_string(dispensed+"g",LCD_LINE_2,2)
        lcd_string("R"+str(rands),LCD_LINE_3,2)       
        lcd_string("Return lever to end",LCD_LINE_4,2)

      
    if GPIO.input(START) != GPIO.LOW:
            
        dispensed = start_weight - hx.get_weight_mean(40)
        rands = float(dispensed/gperR)
        rands = Decimal(rands).quantize(Decimal('.01'), rounding=ROUND_DOWN)
...
def main():
    ...
    while GPIO.input(START) != GPIO.LOW:
        hx.zero()
        start_weight = hx.get_weight_mean(20)
        start_weight = round(start_weight, 0)
        start_weight = max(0, start_weight)
        print(start_weight)
